...
export function CheckOtpForm(props: CheckOtpFormProps): JSX.Element {
	const { form, error, onSubmit, onResend, isLoading, secondsLeft, canResend } = useCheckOtp(props);
	...
	return (
		<form onSubmit={onSubmit} noValidate>
			...
			{!canResend ? (
				<HelperText size='sm'>
					Запросить код повторно можно через {` ${secondsLeft} ${declineRuSeconds(secondsLeft, ['секунду', 'cекунды', 'секунд'])}`}
				</HelperText>
			) : (
