...
import { isErrorWithMessage } from '@/shared/api/errors';
		...
		(set, get) => ({
			...
			signIn: async (code) => {
				...
				try {
					...
					set({
						isLoading: false,
						error: isErrorWithMessage(err) ? err.message : 'Произошла ошибка аутентификации'
					});
					return false;
				}
			},
			...
			initializeAuth: async () => {
				...
				try {
					...
					if (!token) {
						set({
							user: null,
							status: status === 'otp_sent' ? 'otp_sent' : 'anonymous',
						});
						return;
					}
